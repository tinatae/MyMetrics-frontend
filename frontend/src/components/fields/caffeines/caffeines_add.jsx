...
class Caffeines extends React.Component {
  constructor(props) {
    super(props);
    ...
    if (this.props.location.pathname === `/${FieldRoutes[category]}`) {
      return (
        <div id="field-logo">
          ...
          <span>Caffeine ({new Date(Date.now()).toLocaleDateString('en-US', { weekday: 'short', month: 'numeric', day: 'numeric' }).split(',').join(' ')})</span>        
        </div>
      );
    } else {
      return (
  ...
  handleSubmit(e) {
    ...
    this.props.createCaffeine(newCaffeine)
    ...
    if (this.state.filled !== false && this.props.location.pathname === "/new_day") {
      return (
        <div id="ask-routes">
          ...
          &nbsp;or&nbsp;
          <button onClick={this.exploreCategory("Caffeine")}>
            Explore Page
          </button>
          &nbsp;or feel free to stay here & fill-out more metrics!
        </div>
      );
    } else if (this.state.filled !== false && this.props.location.pathname === "/caffeine") {
      return (
      <div id="ask-routes">
        Metric has been successfully created! Check it out on your&nbsp;
        <button onClick={this.goToProfile(user)}>Profile</button>     
  ...
  render() {
    ...
    return (
      <div className="caffeine field-add" style={{backgroundColor: FieldColors["Caffeine"]}}>
        <form onSubmit={this.handleSubmit}>
          <div className="form">
            {this.showLogo(category)}
            <div id="add-inputs">
              <div>Daily Caffeine Intake</div>
              <div>
                <select
                  name="Caffeine"
                  value={amt}
...
export default withRouter(Caffeines);
