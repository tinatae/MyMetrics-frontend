...
class Sugars extends React.Component {
  constructor(props) {
    super(props);
    ...
    if (this.props.location.pathname === `/${FieldRoutes[category]}`) {
      return (
        <div id="field-logo">
          ...
          <span>Sugar ({new Date(Date.now()).toLocaleDateString('en-US', { weekday: 'short', month: 'numeric', day: 'numeric' }).split(',').join(' ')})</span>
        </div>
      );
    } else {
      return (
  ...
  handleSubmit(e) {
    ...
    this.props.createSugar(newSugar)
    ...
    if (this.state.filled !== false && this.props.location.pathname === "/new_day") {
      return (
        <div id="ask-routes">
          ...
          &nbsp;or&nbsp;
          <button onClick={this.exploreCategory("Sugar")}>Explore Page</button>
          &nbsp;or feel free to stay here & fill-out more metrics!
        </div>
      )
    } else if (this.state.filled !== false && this.props.location.pathname === "/sugar") {
      return (
        <div id="ask-routes">
          Metric has been successfully created! Check it out on your&nbsp;
          <button onClick={this.goToProfile(user)}>Profile</button>     
  ...
  render() {
    ...
    return (
      <div className="sugar field-add" style={{backgroundColor: FieldColors["Sugar"]}}>
        <form onSubmit={this.handleSubmit}>
          <div className="form">
            {this.showLogo(category)}

            <div id="add-inputs">
              <div>Daily Sugar Intake</div>
              <div>
                <select name="Sweets" value={amt} onChange={this.update("amt")}>
                  ...
                  <option value="12">Baker's Dozen</option>
                </select>
                <select
                  id="unit"
                  name="Unit"
...
export default withRouter(Sugars);
