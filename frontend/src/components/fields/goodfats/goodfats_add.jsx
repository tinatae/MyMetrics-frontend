...
class GoodFats extends React.Component {
  constructor(props) {
    super(props);
    ...
    if (this.props.location.pathname === `/${FieldRoutes[category]}`) {
      return (
        <div id="field-logo">
          ...
          <span>Good Fats ({new Date(Date.now()).toLocaleDateString('en-US', { weekday: 'short', month: 'numeric', day: 'numeric' }).split(',').join(' ')})</span>
        </div>
      );
    } else {
      return (
  ...
  handleSubmit(e) {
    ...
    this.props.createGoodFat(newGoodFat)
    ...
    if (this.state.filled !== false && this.props.location.pathname === "/new_day") {
      return (
        <div id="ask-routes">
          ...
          &nbsp;or&nbsp;
          <button onClick={this.exploreCategory("GoodFat")}>Explore Page</button>
          &nbsp;or feel free to stay here & fill-out more metrics!
        </div>
      )
    } else if (this.state.filled !== false && this.props.location.pathname === "/goodfats") {
      return (
        <div id="ask-routes">
          Metric has been successfully created! Check it out on your&nbsp;
          <button onClick={this.goToProfile(user)}>Profile</button>
  ...
  render() {
    ...
    return (
      <div className="goodfat field-add" style={{backgroundColor: FieldColors["GoodFat"]}}>
        <form onSubmit={this.handleSubmit}>
          <div className="form">
            ...
            <div id="add-inputs">
              ...
              <div>
                <select name="GoodFats" value={amt} onChange={this.update("amt")}>
...
export default withRouter(GoodFats);
